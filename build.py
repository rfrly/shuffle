...
watch_css = r"""
    ...
    .obs-menu-audio-icon.on { opacity: 1; }
    .observer-info-strip {
...
firebase_and_observer = r"""
    ...
    function ObserverDisplay({ state, code, onDisconnect, onSendCmd }) {
      ...
      const [menuConfirm, setMenuConfirm] = React.useState(null);
      const menuConfirmTimer = React.useRef(null);
      ...
      const [teacherAudio, setTeacherAudio] = React.useState(() => {
        try { return localStorage.getItem("shuffle_watch_teacherAudio") === "1"; } catch { return false; }
      });
      const [teacherRunning, setTeacherRunning] = React.useState(false);
      const [teacherPaused, setTeacherPaused] = React.useState(false);
      const [teacherResuming, setTeacherResuming] = React.useState(false);
      const [teacherLooping, setTeacherLooping] = React.useState(false);
      ...
      // ── Watch 1.14: sync teacher scheduler with student-initiated transport ─
      // Each effect is a no-op on teacher-initiated roundtrips because the teacher
      // handler sets teacher-local state synchronously alongside sending the cmd,
      // so by the time obs* echoes back, the guards already hold.
      React.useEffect(() => {
        if (!teacherAudio) return;
        if (obsRunning && !teacherRunning) {
          try { getTeacherCtx().resume().catch(() => {}); } catch {}
          setTeacherPaused(false);
          setTeacherResuming(false);
          setTeacherLooping(false);
          setTeacherRunning(true);
          _db.ref("sessions/" + code + "/cmds").set({ audioMuted: true, tseq: Date.now() });
        } else if (!obsRunning && teacherRunning) {
          setTeacherRunning(false);
          setTeacherPaused(false);
          setTeacherResuming(false);
          setTeacherLooping(false);
          _db.ref("sessions/" + code + "/cmds").set({ audioMuted: false, tseq: Date.now() });
        }
      }, [obsRunning, teacherAudio]);

      React.useEffect(() => {
        if (!teacherAudio || !teacherRunning) return;
        if (!!obsPaused === !!teacherPaused) return;
        if (obsPaused) {
          setTeacherPaused(true);
          setTeacherResuming(false);
        } else {
          setTeacherPaused(false);
          setTeacherResuming(true);
        }
      }, [obsPaused, teacherAudio, teacherRunning]);

      React.useEffect(() => {
        if (!teacherAudio || !teacherRunning) return;
        if (!!obsLooping === !!teacherLooping) return;
        setTeacherLooping(!!obsLooping);
      ...
      // Teacher's own audio scheduler — dormant (running=false) when teacherAudio is off.
      // Called unconditionally to satisfy React hooks rules. Callbacks are no-ops because
      // the teacher display reads from observedState, not from this scheduler's output.
      const { getCtx: getTeacherCtx } = useDrumTimer({
        bpm: obsBpm || 80,
        beatsPerBar: parseInt((obsTimeSigLabel || "4/4").split("/")[0]) || 4,
        barsPerExercise: (obsBpe || 4) * (obsExMode === 'pick' ? 1 : (obsExLen || 1)),
        minEx: obsMinEx || 1, maxEx: obsMaxEx || 4,
        onNewExercise: () => {}, onNextExercise: () => {},
        onSetComplete: () => {}, onSetLoop: () => {},
        running: teacherRunning, paused: teacherPaused, resuming: teacherResuming,
        countInBars: obsCib || 1, countInEveryRound: !!obsCountInEvery,
        mode: obsMode || "fullset",
        volume: obsVolume ?? 1,
        looping: teacherLooping,
        infinite: obsSets === '\u221e' || (typeof obsSets === 'number' && obsSets > 1),
        setComplete: false,
        exMode: obsExMode || "range", pickedNums: obsPickedNums || [],
        subdivision: obsSubdivision || 1, beatStates: Array.isArray(obsBeatStates) ? obsBeatStates : [],
        subdivVol: obsSubdivVol ?? 0.7, subdivVol2: obsSubdivVol2 ?? 0.7, subdivVol3: obsSubdivVol3 ?? 0.7,
        metSound: obsMetSound || "digital1",
        keepCtxAlive: teacherAudio,
      });
        ...
        <div className="observer-app">
          <div className="watching-banner">
            <div className="watching-banner-right">
              ...
              <button className="watching-disconnect-btn" onClick={() => { setTeacherRunning(false); setTeacherPaused(false); setTeacherResuming(false); setTeacherLooping(false); setTeacherAudio(false); onSendCmd({ tcmd: "end-session", tseq: Date.now() }); _db.ref("sessions/" + code).remove(); onDisconnect(); }}>End session</button>
              <span className={`obs-menu-audio-icon${teacherAudio ? " on" : ""}`}>
                <svg width="10" height="10" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg" style={{display:"block"}}>
                  <polygon points="1,4 5,4 9,1 9,13 5,10 1,10" fill="#888"/>
                  <path d="M10.5 4.5 C12 5.5 12 8.5 10.5 9.5" stroke="#888" strokeWidth="1.4" strokeLinecap="round" fill="none"/>
                </svg>
              </span>
            </div>
            <button className={`obs-menu-btn${menuOpen ? " open" : ""}`}
              onClick={() => setMenuOpen(v => !v)}><span style={{marginTop:"-2px"}}>☰</span></button>
            {menuOpen && (
              <>
                ...
                <div className="obs-menu-panel">
                  <button className="obs-menu-item obs-menu-item--toggle" onClick={() => {
                    const next = !teacherAudio;
                    setTeacherAudio(next);
                    try { localStorage.setItem("shuffle_watch_teacherAudio", next ? "1" : "0"); } catch {}
                    if (!next) {
                      ...
                      try { getTeacherCtx().resume().catch(() => {}); } catch {}
                      if (teacherRunning) onSendCmd({ audioMuted: true });
                    }
                    ...
                    <div className="settings-menu-submenu">
                      {[
                        ...
                        <button key={id}
                          ...
                          onClick={() => {
                            onSendCmd({ metSound: id });
                            try {
                              if (teacherAudio) {
                                const ctx = getTeacherCtx();
                                if (ctx) scheduleMetronomeClick(ctx, ctx.currentTime + 0.02, 'accent', obsVolume ?? 1, false, id);
                              } else {
                                const previewCtx = new (window.AudioContext || window.webkitAudioContext)();
                                scheduleMetronomeClick(previewCtx, previewCtx.currentTime + 0.02, 'accent', obsVolume ?? 1, false, id);
                  ...
                  <button className="obs-menu-item obs-menu-item--action" onClick={() => {
                    const summaryParts = [];
                    if (obsMode !== "clickonly") {
                    ...
                    navigator.clipboard.writeText(`${summaryParts.join(", ")}\n${url}`)
                      .then(() => { showMenuConfirm("share"); showToast("Copied to clipboard!"); })
                      .catch(() => { showMenuConfirm("share-fail"); showToast("Copy failed"); });
                  }}><span>Share settings</span>{menuConfirm === "share" && <span className="obs-menu-confirm">✓</span>}{menuConfirm === "share-fail" && <span className="obs-menu-confirm" style={{color:"#a04040"}}>✗</span>}</button>
                  <button className="obs-menu-item obs-menu-item--action obs-menu-item--destructive" onClick={() => {
                    setTeacherRunning(false); setTeacherPaused(false); setTeacherResuming(false); setTeacherLooping(false);
                    ...
                    showMenuConfirm("reset"); showToast("Settings reset");
                  }}><span>Reset to defaults</span>{menuConfirm === "reset" && <span className="obs-menu-confirm" style={{color:"#a04040"}}>✓</span>}</button>
                </div>
              </>
            )}
          </div>
          ...
          <div className="btn-row" style={{ width: "100%" }}>
            {!obsRunning ? (
              <button className="action-btn" disabled={disabled || !validRange}
                onClick={() => {
                  ...
                  if (teacherAudio) {
                    ...
                    let started = false;
                    const startTeacher = (warn) => { if (!started) { started = true; setTeacherRunning(true); if (warn) showToast("Connection issue \u2014 check student audio", 4000); } };
                    _db.ref("sessions/" + code + "/cmds").set({ tcmd: "start", tseq: T, audioMuted: true })
                      .then(() => startTeacher(false)).catch(() => startTeacher(true));
                    setTimeout(() => startTeacher(true), 500);
                  } else {
                    onSendCmd({ tcmd: "start", tseq: T });
                  }
                }}>Start</button>
            ) : obsMode === "clickonly" ? (
              <button className="action-btn stop" disabled={disabled}
                onClick={() => {
                  if (teacherAudio) {
                    onSendCmd({ tcmd: "stop", tseq: Date.now(), audioMuted: false });
                    setTeacherRunning(false); setTeacherPaused(false); setTeacherResuming(false); setTeacherLooping(false);
                  } else { onSendCmd({ tcmd: "stop", tseq: Date.now() }); }
                }}>Stop</button>
            ) : (
              <>
                <div className="btn-group">
                  <button className={`action-btn${obsPaused ? " pause-active" : " secondary"}`} disabled={disabled}
                    onClick={() => {
                      const isResume = obsPaused;
                      onSendCmd({ tcmd: isResume ? "resume" : "pause", tseq: Date.now() });
                      if (teacherAudio) {
                        if (isResume) { setTeacherResuming(true); setTeacherPaused(false); }
                        else { setTeacherPaused(true); setTeacherResuming(false); }
                      }
                    }}>
                  ...
                  <button className={`action-btn${obsLooping ? " loop-active" : " secondary"}`} disabled={disabled}
                    onClick={() => {
                      onSendCmd({ tcmd: "loop", tseq: Date.now() });
                      if (teacherAudio) setTeacherLooping(l => !l);
                    }}>Loop</button>
                </div>
                <div className="btn-group-stop">
                  <button className="action-btn stop" disabled={disabled}
...
watch_state = """
      ...
      const [studentAudioMuted, setStudentAudioMuted] = useState(false);
...
watch_effects = """      // ── Watch: manage silent loop to keep AudioContext alive while sharing ──
      ...
      const handleStartSharing = useCallback(() => {
        ...
        setExercise(null); setNextEx(null); setSetCount(1);
      ...
      useEffect(() => {
        ...
        cmdsRef.on("value", snap => {
          ...
          if (cmd.audioMuted != null) setStudentAudioMuted(!!cmd.audioMuted);
        });
      ...
      return (
        ...
        {watchScreen === "home" && (
          <div className="watch-overlay">
            ...
            <div style={{ fontSize: "0.55rem", color: "#444", fontFamily: "var(--font-mono)", letterSpacing: "0.1em", marginTop: "0.5rem" }}>v1.10.10 · watch 1.27</div>
          </div>
        )}
        {watchScreen === "share" && (
          <div className="watch-overlay">
...
teacher_parity_checks = [
    ...
    # Subdivision (accessible via Vol popup — not in controls grid)
    ("vol-subdiv-icon-btn",     "teacher Subdivision accessible via Vol popup"),
    ...
    # Teacher audio mode
    ("Teacher Audio",           "teacher audio toggle label in \u2630 menu"),
    ("teacherAudio",            "teacher audio mode state var"),
    ("audioMuted",              "student cmd listener handles audioMuted"),
    ("studentAudioMuted",       "student audio mute state var"),
]

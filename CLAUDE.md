...
### Version numbering
The watch app has its own version number separate from the main app, displayed on the home screen as e.g. `v1.8.1 · Watch 1.0`. The `WATCH_VERSION` constant is at the top of `build-watch.sh`. Increment it when shipping watch-specific changes:
- Significant new watch features: increment minor (1.0 → 1.1)
- Bug fixes and polish: increment patch (1.0 → 1.0.1)
The main app version shown alongside it is extracted automatically from `test/index.html` at build time (beta suffix stripped).

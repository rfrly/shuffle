...
- When the student taps "Share my session", settings reset to defaults (BPM 100, 4/4, 1-bar count-in, exercises 1–4, shuffle mode, range mode) so each session starts clean
...
- **iOS audio note:** The student must tap "Open Shuffle" before the teacher starts — this is the required user gesture to unlock the Web Audio context. On tap, a looping near-silent buffer (`watchSilentLoop` ref) is started immediately to keep the context alive while waiting for the teacher to start (iOS re-suspends the context if no audio is playing). `useDrumTimer` takes over with its own silent loop once `running` becomes true. If the teacher starts before the student taps, the metronome will hang silently. The green "Teacher connected" prompt exists specifically to prompt this tap.
- **Known issue (unresolved):** When the teacher taps Start, it still hangs on the first beat — iOS won't resume the audio context from a Firebase listener callback (not a user gesture). Workaround: student taps Start themselves. **Next fix to try:** call `getCtx().resume().catch(()=>{})` inside the `tcmd === "start"` handler in `build-watch.sh`, immediately before `setRunning(true)`.
...
### Version numbering
The watch app has its own version number displayed on the home screen (e.g. `v1.8.1 · watch 1.1`). The first part mirrors the main app version; the watch version increments independently. **Update the watch version in `build-watch.sh` every time any watch-related change is made** (the string is in the home screen JSX near the bottom of the watch_jsx block).

...
### What it does
- Student opens `shuffleclick.com/watch/` → taps "Share my session" → gets an 8-character code → uses Shuffle normally
- Teacher opens `shuffleclick.com/watch/` on their device → taps "Watch a session" → enters the code → sees a live read-only view of the student's session (exercise number, beat dots, bar progress, BPM, mode)
...
### How it's built
`watch/index.html` is the full Shuffle app (copied from `test/index.html`) with the following injected on top:
1. Firebase SDK script tags (compat library, Realtime Database)
2. Watch-specific CSS (home/share/watch-entry/observer screens)
3. Firebase init + `ObserverDisplay` React component
4. Watch mode state variables in `App` (`watchScreen`, `shareCode`, `observedState`, etc.)
5. `useEffect` that broadcasts live playback state to Firebase when sharing
6. Watch handlers (`handleStartSharing`, `handleStopSharing`, `handleConnectWatch`, `handleDisconnectWatch`)
7. Watch UI overlays wrapping the JSX return (home/share/watch-entry screens; observer view renders instead of the main app when watching)
...
```
This rebuilds `watch/index.html` from scratch. Commit both `test/index.html` and the regenerated `watch/index.html` together.
...
- Security rules: sessions are readable by anyone, writable only on creation; auto-deleted via `onDisconnect().remove()` when the student leaves. Rules to apply in Firebase console:
  ```json
  {
    "rules": {
      "sessions": {
